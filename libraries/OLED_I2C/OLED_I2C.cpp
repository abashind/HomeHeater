/*
  OLED_I2C.cpp - Arduino/chipKit library support for 128x32 and 128x64 pixel SSD1306 OLEDs
  Copyright (C)2015-2018 Rinky-Dink Electronics, Henning Karlsen. All right reserved
  
  This library has been made to make it easy to use 128x32 and 128x64 pixel OLED displays
  based on the SSD1306 controller chip with an Arduino or a chipKit.

  You can always find the latest version of the library at 
  http://www.RinkyDinkElectronics.com/

  This library is free for non-commercial use; you can redistribute it
  and/or modify it under the terms of the CC BY-NC-SA 3.0 license.
...
boolean OLED::begin(uint8_t dtype)
{
	switch(dtype)
	{
	case SSD1306_128X32:
		_dheight = H32_DISPLAY_HEIGHT;
		_mpratio = H32_MULTIPLEX_RATIO;
		_compins = H32_COM_PINS;
		_bufsize = H32_BUFFER_SIZE;
		scrbuf = (uint8_t*) malloc(_bufsize);
		break;
	case SSD1306_128X64:
		_dheight = H64_DISPLAY_HEIGHT;
		_mpratio = H64_MULTIPLEX_RATIO;
		_compins = H64_COM_PINS;
		_bufsize = H64_BUFFER_SIZE;
		scrbuf = (uint8_t*) malloc(_bufsize);
		break;
	}
	if (scrbuf == NULL)
    ...
    _sendTWIcommand(_mpratio);			// 0x3F for 128x64, 0x1F for 128x32
    ...
    _sendTWIcommand(_compins);			// 0x12 for 128x64, 0x02 for 128x32
...
void OLED::clrScr()
{
	memset(scrbuf, 0, _bufsize);
}

void OLED::fillScr()
{
	memset(scrbuf, 255, _bufsize);
}
...
void OLED::rotateDisplay(bool rot)
{
	if (rot)
	{
		_sendTWIcommand(SSD1306_SET_SEGMENT_REMAP);
		_sendTWIcommand(SSD1306_COM_SCAN_DIR_INC);
	}
	else
	{
		_sendTWIcommand(SSD1306_SET_SEGMENT_REMAP | 0x1);
...
void OLED::drawHLine(int x, int y, int l)
{
	...
	if ((x>=0) and (x<128) and (y>=0) and (y<_dheight))
	{
